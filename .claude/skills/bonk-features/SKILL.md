...
- **Raakt / let op:**
  ...
  - Gebruikt `nextEvent` → herinneringen tellen mee. `nextEvent`/`upcoming` zijn al beperkt tot het dagvenster (zie feature 1b), dus de oude `menuBarOnlyToday`-check is wèg uit `menuBarText`.

### 1b. Weergavevenster — "toon X dagen" + max aantal meetings
- **Wat:** instelbaar hoeveel er getoond wordt in **menubalk én menu**: een dagvenster `displayDays` (1 = alleen vandaag, default) en optioneel een maximum aantal **agenda-meetings** `maxMeetings` (nil = alle, default). Herinneringen tellen **niet** mee voor het max en blijven altijd zichtbaar binnen het venster. (Verving de oude `menuBarOnlyToday`-toggle.)
...
- **Raakt / let op:** agenda-kleur-logica is gespiegeld met `MenuView.calendarColor` (houd ze gelijk). Contrast-voorgrond via `Color.readableForeground`. Werkt ook voor herinneringen (geen agenda → accent paars). Respecteert `globalEnabled` en — via de al-gefilterde `next`/`upcoming` — het dagvenster `displayDays` (feature 1b).
...
- **Herhalen (alarm):** macOS heeft geen lange/alarm-geluiden; daarom kan een schermvullende regel het geluid **herhalen tot je reageert** (`MeetingRule.repeatSound` / `reminderRepeatSound`). `AlertSound.play(_,repeating:)` zet `NSSound.loops = true` (Standaard→`Sosumi`), met cap 30s; `AlertSound.stop()` stopt het bij join/snooze/sluiten (in `present`-closures) én bij overlay-`close()`. Alleen schermvullend (een notificatie kun je niet stoppen).
- **Spelen bij gedempte Mac:** `MeetingRule.overrideMute` / `reminderOverrideMute` → `AlertSound.play(_,forceAudible:)`. macOS kan een gewone app niet "door" mute heen laten spelen, dus `SystemAudio.forceAudible()` (AppleScript `set volume`) haalt de Mac tijdelijk uit mute / zet het volume ≥35, en `AlertSound` herstelt de oude staat na het geluid (eenmalig: na `duration`; herhalend: bij `stop()`). Geen TCC nodig (`set volume` draait in de osascript zelf).
