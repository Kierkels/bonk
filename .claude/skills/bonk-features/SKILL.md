...
- **Waar:** `AppDelegate.menuBarText`, `BonkApp.MenuBarLabel`. Instellingen: **Weergave**-tab → sectie "Menubalk".
- **Raakt / let op:**
...
- **Wat:** kop met globale aan/uit-toggle; "Volgende meeting(s)" (meerdere tegelijk = primair), "Daarna", "Genegeerd"; per kaart join-knop, **open-in-agenda (📅)**, negeer/verwijder (✕), herinnering bewerken (✏️); knoppen Herinnering toevoegen / Instellingen / Afsluiten; **update-banner** bovenaan. Per-agenda kleurstreepje.
...
- **Menu-weergave:** is er een sneltoets ingesteld, dan toont het menu die als accelerator achter "Herinnering toevoegen…" (`MenuView.footer`, trailing `shortcut.displayString`).
